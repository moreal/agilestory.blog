...
import { extractSnippet } from "../lib/snippet.ts"
...
export function SearchPage({ posts }: SearchPageProps) {
  ...
  const [status, setStatus] = useState<"no-query" | "no-results" | "success">("no-query")

  // Filter valid posts on component mount
  const validPosts = useMemo(() => posts.filter((post) => post.createdAt !== null), [posts])
        ...
        {status === "no-query" && (
          <div className="text-center py-16">
            <div className="max-w-md mx-auto">
              <h2 className="text-xl font-medium text-gray-900 mb-2">검색을 시작해보세요</h2>
              <p className="text-gray-600">찾고 싶은 내용의 키워드를 입력해 주세요.</p>
            </div>
          </div>
        )}
          ...
          <div className="text-center py-16">
            <div className="max-w-md mx-auto">
              <h2 className="text-xl font-medium text-gray-900 mb-2">
                "<span className="text-blue-600">{query}</span>" 검색 결과가 없습니다
              </h2>
        ...
        {status === "success" && (
          <div>
            <div className="mb-8">
              ...
              <p className="text-sm text-gray-500">{searchResults.length}개 발견</p>
            </div>
