...
function NavigationItem({
  ...
  return (
    <li class={`flex-1 ${textOrderClass}`}>
      ...
      >
        <div class="text-xs text-blue-600 font-medium mb-1">{navigationMessage}</div>
        <p class="text-sm font-medium text-gray-900 line-clamp-2">{title}</p>
      </a>
    </li>
  )
}
...
function NavigationItemOrSpacer({
  ...
  return <NavigationItem post={post} navigationMessage={navigationMessage} textOrder={textOrder} />
}
      ...
      <div class="max-w-4xl mx-auto">
        <ul class="flex gap-4">
          <NavigationItemOrSpacer
            post={prevPost}
            navigationMessage={`← ${site.messages.previousPost}`}
            textOrder="left"
          />
          <NavigationItemOrSpacer
            post={nextPost}
            navigationMessage={`${site.messages.nextPost} →`}
            textOrder="right"
          />
        </ul>
      </div>
